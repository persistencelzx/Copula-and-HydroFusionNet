...
for i = 1:numPairs
    ...
    groupName = string("Pair_" + colX + "_" + colY);
groupNames = repmat(groupName, 5, 1);  % 5x1

models = {'Gaussian'; 't'; 'Frank'; 'Gumbel'; 'Clayton'};  % 5x1 cell
rhos = [rho_G; rho_t_corr; rho_Frank; rho_Gumbel; rho_Clayton];  % 5x1 double
dfs = [NaN; nuhat; NaN; NaN; NaN];  % 5x1
BICs = [BIC_G; BIC_t; BIC_F; BIC_Gu; BIC_C];  % 5x1
RMSEs = [RMSE_G; RMSE_t; RMSE_F; RMSE_Gu; RMSE_C];  % 5x1

% AIC
AIC_G = -2 * sum(log(C_Gaussian)) + 2 * 1;
AIC_t = -2 * sum(log(C_t)) + 2 * 2;
AIC_F = -2 * sum(log(C_Frank)) + 2 * 1;
AIC_Gu = -2 * sum(log(C_Gumbel)) + 2 * 1;
AIC_C = -2 * sum(log(C_Clayton)) + 2 * 1;

% 整合进表格
AICs = [AIC_G; AIC_t; AIC_F; AIC_Gu; AIC_C];  % 5x1

tempT = table(groupNames, models, rhos, dfs, AICs, BICs, RMSEs, ...
              'VariableNames', {'Pair', 'Model', 'Rho', 'DegreeFreedom', 'AIC', 'BIC', 'RMSE'});
